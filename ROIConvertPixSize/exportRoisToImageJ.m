...
%
function exportRoisToImageJ(roiStruct, outputPath) 
    ...
    for i = 1:length(roiStruct)
        ...
        fwrite(f, max(r.mnCoordinates(:,1)),  'uint16','b'); % 12-13   bottom
        fwrite(f, max(r.mnCoordinates(:,2)),  'uint16','b'); % 14-15   right
        fwrite(f, size(r.mnCoordinates,1),  'uint16','b');   % 16-17   NCoordinates
        ...
        fwrite(f, reshape(uint16(r.mnCoordinates),[],1), 'uint16','b');
    ...
    % zip it all up!
    zip(strcat(outputPath, "\New_RoiSet.zip"), fNames)    
