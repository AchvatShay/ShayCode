...
function [y_den, psnr_val, ssim_val] = external_BM3DPCA(y, z, sigma, data)

%external_BM3DPCA: BM3DPCA denoising using a targeted external database
%Input:
...
for row = 1:Nstep:height
    for col = 1:Nstep:width
        ...
        [U,~,~] = svd(array2D*diag(W_vec)*array2D');
        subDim = 10; 
        U = U(:,1:subDim);
        blk_ref = blk_ref - mu; 
        spec_ref = U'*blk_ref;
        if bWiener == 0
            spec_ref(abs(spec_ref) < hThld*sigma) = 0;
        else
            blk_est = blk_est(:);
            blk_est = blk_est - mu;
            spec_est = U'*blk_est;            
            spec_ref = spec_ref.*((spec_est.^2)./(spec_est.^2+sigma^2));
        end
        rec_ref = U*spec_ref;
        rec_ref = rec_ref + mu;
