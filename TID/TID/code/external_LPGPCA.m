...
function [y_den, psnr_val, ssim_val] = external_LPGPCA(y, z, sigma, data)

%external_LPGPCA: LPGPCA denoising using a targeted external database
%Input:
...
for row = 1:Nstep:height
    for col = 1:Nstep:width
        ...
        blk_ref = blk_ref - mu; 
        spec_ref = U'*blk_ref;
        spec_ref = spec_ref.*(max(spec_ref.^2-sigma^2, 0)./spec_ref.^2);
